...
class EventsMixin:
    ...
    async def on_friend_message(self, event: AstrMessageEvent):
        ...
        # 对话增强：用户真实发言 → 取消待执行的追发任务，并重置连续追发链
        followup_cancelled = self._cancel_followup(normalized_session_id)
        if followup_cancelled:
            logger.info(
                f"[对话增强] {self._get_session_log_str(normalized_session_id)} 已发言，取消待执行的追发任务喵。"
            )
        async with self.data_lock:
            payload = self.session_data.get(normalized_session_id)
            if payload and payload.get("followup_chain_count"):
                payload["followup_chain_count"] = 0
